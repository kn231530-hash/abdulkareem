...
export async function POST(request: Request) {
  ...
  try {
    ...
    const response = await fetch("https://api.groq.com/openai/v1/chat/completions", {
      ...
      body: JSON.stringify({ model: "openai/gpt-oss-120b", messages: [{ role: "system", content: SYSTEM_PROMPT }, ...messages], temperature: 0.4, max_tokens: 1024 })
    });
    const data = await response.json();
    if (!response.ok) throw new Error(data?.error?.message || "Groq request failed");
    return NextResponse.json({ message: data.choices?.[0]?.message?.content || "Sorry, mujhe response nahi mila." });
  } catch (error) {
    console.error("Groq error:", error);
    return NextResponse.json({ error: error instanceof Error ? error.message : "Groq request failed." }, { status: 500 });
  }
}